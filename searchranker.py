...
class PaperSearchRanker:
    def __init__(self, q, spark, embeddings_path="data/bert_embeddings.parquet", index_path="data/inverted_index.parquet",
    ...
    def search_and_rank(self, query, top_k=3):
        ...
        self.q.put({"step": f"Query tokens: {query_tokens}"})
        ...
        self.q.put({"step": f"Candidate papers: {candidates.count()}"})
        ...
        self.q.put({"step": f"Candidates with embeddings: {candidates_df.count()}"})
