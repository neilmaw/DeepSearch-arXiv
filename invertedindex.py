...
class InvertedIndex:
    ...
    def build(self, testkeywords=[]):
        ...
        if len(testkeywords) == 0:
            ...
            rows = inverted_index_df.limit(5).collect()
            for row in rows:
                self.q.put({"step": f"{row}"})
                self.q.put({"step": "======================================="})
        else:
            inverted_index_df.filter(inverted_index_df.token.isin(testkeywords)).show(5, truncate=False)
            rows = inverted_index_df.filter(
                inverted_index_df.token.isin(testkeywords)
            ).limit(5).collect()
